...
@section('description', 'Ghana School of Law – Your official pathway to the Ghana Bar. LPTC, Post-Call Course, Bar Exam Preparation and Specialised Professional Development under CLET Act 1170.')
@section('content')
        ...
        <div class="flex gap-4 flex-wrap items-center">
            <a href="https://forms.gslaw.school/surveys/23" target="_blank" rel="noopener"
               class="inline-flex items-center gap-2 px-7 py-3.5 text-[14px] font-semibold
                ...
                Apply Now
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </a>
...
<div class="border-t border-b border-gray-100 bg-white">
    <div class="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
        @foreach([
            ['961',    'Called to the Bar 2025',    'Main call 824 · Mini call 137'],
            ['513',    'Female Lawyers Called 2025', 'Main call 453 · Mini call 60'],
            ['12,226', 'Lawyers Enrolled to Bar',   'Since GSL was founded in 1963'],
        ] as $stat)
        <div class="flex flex-col items-center text-center px-8 py-10">
            <span class="font-serif font-bold text-gold leading-none mb-2" style="font-size:clamp(40px,5vw,64px)">{{ $stat[0] }}</span>
            <span class="text-[12px] font-semibold text-navy tracking-[1.5px] uppercase mb-1">{{ $stat[1] }}</span>
        ...
        <div>
            ...
            <ul class="flex flex-col gap-3 mb-8">
                ...
                <li class="flex items-start gap-3 text-[14px] text-navy/70 leading-[1.6]">
                    <span class="mt-1 w-5 h-5 rounded-full border border-gold bg-gold/10 flex items-center justify-center flex-shrink-0">
                        <svg class="w-2.5 h-2.5" fill="none" stroke="#b8960c" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                    </span>
                    {{ $pt }}
                </li>
                @endforeach
...
<div class="border-t border-b border-gold/10 py-12 px-[5%] bg-navy">
    <div class="max-w-6xl mx-auto flex flex-wrap items-center justify-center">
        ...
        <div class="flex flex-col items-center text-center px-6 sm:px-10 py-4">
            <div class="w-12 h-12 rounded-full border-2 border-gold flex items-center justify-center font-serif font-bold text-[18px] text-gold mb-3">{{ $s[0] }}</div>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4 mb-10">
            ...
            <div class="flex items-center gap-3">
                <button data-carousel-prev class="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Previous programme">
                ...
                <button data-carousel-next class="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Next programme">
        ...
        {{-- Track --}}
        <div data-carousel-track class="flex overflow-x-auto hide-scrollbar snap-x snap-mandatory rounded-xl border border-gold/8 bg-gold/8">
            @foreach([
            ...
            <div data-carousel-slide
                ...
                @if($p[6])
                <span class="absolute top-4 right-5 text-[10px] font-bold bg-gold text-navy px-2.5 py-1 rounded-full tracking-[1px] uppercase">Only at GSL</span>
                @endif
                <p class="text-[11px] font-bold text-gold/45 tracking-[3px] uppercase mb-4">Programme {{ $p[0] }}</p>
                <div class="w-12 h-12 rounded-lg bg-gold/8 border border-gold/20 flex items-center justify-center mb-5">
                    <svg class="w-6 h-6 stroke-gold fill-none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">{!! $p[7] !!}</svg>
                </div>
                <h3 class="font-serif font-semibold text-[20px] text-white mb-3 leading-snug">{{ $p[1] }}</h3>
                ...
                <div class="flex flex-wrap gap-x-4 gap-y-1 mb-6">
                    ...
                    <span class="text-[12px] text-cloud/45"><strong class="text-cloud/65 font-semibold">{{ $m[0] }}:</strong> {{ $m[1] }}</span>
                    @endforeach
                </div>
                <a href="{{ route('programmes') }}"
                   class="inline-flex items-center gap-1.5 text-[13px] font-semibold text-gold hover:gap-3 transition-all duration-200">
                    Learn More
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                </a>
            </div>
            @endforeach
        </div>
...
@php
$whyGsl = [
    ['CLET Accredited','Full CLET accreditation with rigorous annual inspections — ensuring Ghana and international quality standards are consistently met.','<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>'],
    ['Expert Faculty','Senior lecturers, practising lawyers, judges and industry leaders bringing decades of real-world Ghanaian legal experience.','<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>'],
    ['Three Campuses','Modern facilities in Accra and Kumasi — each with mock courtrooms, law libraries, computer labs and client suites.','<rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>'],
];
@endphp
<section class="py-24 px-[5%] bg-white">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            @foreach($whyGsl as $c)
            <div class="p-8 rounded-xl bg-white border border-gray-100 shadow-sm hover:border-gold/35 hover:shadow-md hover:-translate-y-1 transition-all duration-200">
                <div class="w-11 h-11 rounded-lg bg-gold/8 border border-gold/20 flex items-center justify-center mb-5">
                    <svg class="w-5 h-5 stroke-gold fill-none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">{!! $c[2] !!}</svg>
                </div>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4 mb-10">
            ...
            <div class="flex items-center gap-2">
                <button data-carousel-prev class="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Previous event">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <button data-carousel-next class="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Next event">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
                <a href="{{ route('events') }}"
        ...
        <div data-carousel-track class="flex gap-5 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-2">
            @foreach([
                ['14','Jan','10:00 – 12:00','Induction Ceremony','New student induction for the 2026/2027 academic year.'],
                ['07','Nov','10:00 – 12:00','Orientation – Kumasi','Orientation session for students at the Kumasi campus.'],
                ['10','Nov','10:00 – 12:00','Call to the Bar','Annual ceremony calling qualified lawyers to the Ghana Bar.'],
            ] as $ev)
            <div data-carousel-slide
                ...
                <div class="flex items-baseline gap-2.5 bg-gold px-5 py-4">
                    <span class="font-serif font-bold text-[38px] text-navy leading-none">{{ $ev[0] }}</span>
                    <span class="text-[13px] font-bold text-navy uppercase tracking-wide">{{ $ev[1] }}</span>
                </div>
                <div class="p-5">
                    <p class="text-[11px] font-bold text-gold/80 tracking-[1.5px] uppercase mb-2">{{ $ev[2] }}</p>
...
<section class="py-24 px-[5%] bg-gray-50" data-carousel="news">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4 mb-10">
            ...
            <div class="flex items-center gap-2">
                <button data-carousel-prev class="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Previous article">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <button data-carousel-next class="w-9 h-9 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy/50 hover:border-gold hover:text-gold shadow-sm transition-all duration-200" aria-label="Next article">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
                <a href="{{ route('news') }}"
                   class="ml-1 px-5 py-2 text-[13px] font-medium border border-navy/20 text-navy/55 rounded hover:border-gold hover:text-gold transition-all duration-200">
        ...
        <div data-carousel-track class="flex gap-5 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-2">
            @foreach([
                ['Institutional','Orientation 2025','The Ghana School of Law successfully held its 2025/2026 orientation for new students across all three campuses.','June 2025'],
                ['Events','Call to the Bar 2025','961 lawyers were called to the Ghana Bar in 2025 — 513 female, marking a historic milestone for gender inclusion.','November 2025'],
                ['Academic','Act 1170 Transformation','GSL formally transitions to its new role as a Directorate of CLET under the Legal Education Act, 2026 (Act 1170).','2026'],
            ] as $art)
            <div data-carousel-slide
                ...
                <div class="h-44 flex items-center justify-center flex-shrink-0"
                     style="background:linear-gradient(135deg,#0c4a6e,#051b2c)">
                    <span class="text-[10px] font-bold text-gold/50 tracking-[3px] uppercase">Ghana School of Law</span>
                </div>
                <div class="p-5 flex flex-col flex-1">
                    <p class="text-[10px] font-bold text-gold tracking-[2px] uppercase mb-2.5">{{ $art[0] }}</p>
                    <h4 class="font-serif font-semibold text-[16px] text-navy leading-snug mb-2 flex-1">{{ $art[1] }}</h4>
...
(function () {
    ...
    document.querySelectorAll('[data-carousel]').forEach(function (section) {
        ...
        function go(index) {
            ...
            track.scrollTo({ left: cur * (slides[0].offsetWidth + gap), behavior: 'smooth' });
            syncDots();
        }
