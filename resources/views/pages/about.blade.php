...
@section('description','Learn about GSL — history, transformation under CLET Act 1170, vision, mission, and governance.')
@section('content')
...
<section class="relative pt-[72px] min-h-[300px] flex items-end pb-14 px-[5%]"
    ...
    <div class="max-w-6xl mx-auto w-full">
        ...
        <h1 class="font-serif font-semibold text-white leading-[1.1] mb-4" style="font-size:clamp(32px,5vw,58px)">Ghana's Premier<br><span class='text-gold'>Legal Training</span> Institution</h1>
        <p class="text-[16px] text-cloud/62 max-w-xl leading-[1.7]">Established in 1963, the Ghana School of Law now operates as a Directorate of CLET under the Legal Education Act, 2026 (Act 1170).</p>
    </div>
</section>
...
<section class="py-20 px-[5%] bg-navy-mid">
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
            ...
            <h2 class="font-serif font-semibold text-white text-[34px] leading-[1.2] mb-5">
            ...
            <p class="text-[15px] text-cloud/70 leading-[1.8]">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="p-6 rounded-xl bg-navy border border-gold/10 text-center">
                <p class="font-serif font-bold text-gold text-[32px] mb-1">{{ $f[0] }}</p>
                <p class="text-[11px] text-cloud/60 uppercase tracking-[1.5px] font-semibold">{{ $f[1] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="py-20 px-[5%] bg-navy">
    <div class="max-w-6xl mx-auto">
        <div class="text-center mb-12">
            ...
            <h2 class="font-serif font-semibold text-white text-[34px]">What Changed Under <span class="text-gold">Act 1170</span></h2>
        </div>
...
<section class="py-20 px-[5%] bg-navy-mid">
    <div class="max-w-6xl mx-auto">
        <div class="text-center mb-12">
            ...
            <h2 class="font-serif font-semibold text-white text-[34px]">Vision, Mission <span class="text-gold">&amp; Values</span></h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach([
                ['Vision','To be Ghana\'s leading centre of professional legal education, producing competent, ethical, and innovative legal practitioners of international standing.'],
                ['Mission','To deliver high-quality professional legal education and training, support Bar Examination candidates, and serve as the gateway for international lawyers entering Ghana\'s legal profession.'],
                ['Values','Excellence · Integrity · Innovation · Inclusion · Collaboration · Service to the Legal Profession and to Ghana.'],
            ] as $v)
            <div class="p-8 rounded-xl border border-gold/15 bg-navy text-center">
                <h3 class="font-serif font-semibold text-[20px] text-white mb-4 pb-4 border-b border-gold/15">{{ $v[0] }}</h3>
                <p class="text-[14px] text-cloud/62 leading-[1.75]">{{ $v[1] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="py-20 px-[5%] bg-navy">
    <div class="max-w-2xl mx-auto text-center">
        ...
        <h2 class="font-serif font-semibold text-white text-[34px] mb-10">GSL within <span class="text-gold">CLET</span></h2>
        <div class="flex flex-col items-center gap-2">
            ...
            @if($i > 0)<div class="w-px h-5 bg-gold/30"></div>@endif
            <div class="w-full max-w-sm px-6 py-4 rounded-lg border {{ $g[2] ? 'border-gold bg-gold/10' : 'border-gold/15 bg-navy-mid' }}">
                <p class="{{ $g[2] ? 'text-gold' : 'text-white' }} font-semibold text-[15px] mb-1">{{ $g[0] }}</p>
