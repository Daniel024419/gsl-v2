...
@section('description','Apply to the Ghana School of Law. Requirements, application process, key dates, and contact information for all programmes.')
@section('content')
...
<section class="relative pt-[72px] min-h-[300px] flex items-end pb-14 px-[5%]"
    ...
    <div class="max-w-6xl mx-auto w-full">
        ...
        <h1 class="font-serif font-semibold text-white leading-[1.1] mb-4" style="font-size:clamp(32px,5vw,58px)">Start Your Journey<br><span class='text-gold'>to the Bar</span></h1>
        <p class="text-[16px] text-cloud/62 max-w-xl leading-[1.7]">Applications for the September 2026 intake are now open. Find out how to apply for each programme.</p>
    </div>
</section>
...
<section class="py-20 px-[5%] bg-navy-mid">
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-10">
            <div>
                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gold/10 border border-gold/25 mb-3">
                    <span class="w-1.5 h-1.5 rounded-full bg-gold"></span>
                    <span class="text-[10px] font-bold text-gold tracking-[2px] uppercase">Applications Open</span>
                </div>
                <h2 class="font-serif font-semibold text-white text-[30px]">Pre-Bar Course 2026/2027</h2>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
                <h3 class="font-serif font-semibold text-[20px] text-white mb-5">Who Can Apply?</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-lg bg-navy border border-gold/8">
                        <div class="w-8 h-8 rounded-full border border-gold bg-gold/10 flex items-center justify-center flex-shrink-0">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="#b8960c" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                        </div>
                        <p class="text-[14px] text-cloud/75">{{ $e }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
            <div>
                <h3 class="font-serif font-semibold text-[20px] text-white mb-5">Course Modules</h3>
                <div class="grid grid-cols-2 gap-2.5 mb-6">
                    ...
                    <div class="px-4 py-3 rounded-lg bg-navy border border-gold/8 text-[13px] text-cloud/70">{{ $m }}</div>
                    @endforeach
                </div>
                <div class="grid grid-cols-2 gap-3">
                    @foreach([['Starts','September 2026'],['Deadline','July 2026'],['Phone','+233 246 006 210'],['Email','helpdesk@gslaw.edu.gh']] as $d)
                    <div class="px-4 py-3 rounded-lg bg-gold/7 border border-gold/15">
                        <p class="text-[10px] font-bold text-gold tracking-[2px] uppercase mb-1">{{ $d[0] }}</p>
                        <p class="text-[13px] text-cloud/80">{{ $d[1] }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="relative p-6 rounded-xl bg-navy-mid border border-gold/10">
                <span class="absolute -top-3 -left-3 w-9 h-9 rounded-full bg-gold text-navy font-bold text-[14px] flex items-center justify-center">{{ $s[0] }}</span>
...
<section class="py-20 px-[5%] bg-navy-mid">
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div>
            ...
            <h2 class="font-serif font-semibold text-white text-[28px] mb-6">Important Deadlines</h2>
            <div class="space-y-3">
                @foreach([['Applications Open','Now — 2026','border-gold/25 bg-gold/8 text-gold'],['Application Deadline','July 2026','border-gold/10 bg-navy text-cloud/70'],['Entrance Examination','August 2026','border-gold/10 bg-navy text-cloud/70'],['Programme Commences','September 2026','border-gold/10 bg-navy text-cloud/70']] as $d)
        ...
        <div>
            ...
            <h2 class="font-serif font-semibold text-white text-[28px] mb-6">Contact Admissions</h2>
            <div class="space-y-3">
                @foreach([['+233 246 006 210','Phone'],[ '+233 531 003 918','Phone'],['helpdesk@gslaw.edu.gh','Email'],['admissions@gslaw.edu.gh','Email']] as $c)
                <div class="flex items-center gap-4 p-4 rounded-lg bg-navy border border-gold/8">
                    <div class="w-9 h-9 rounded-lg bg-gold/10 border border-gold/20 flex items-center justify-center flex-shrink-0">
                        <span class="text-gold font-bold text-[11px] tracking-[1px]">{{ strtoupper(substr($c[1],0,2)) }}</span>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold text-gold/60 tracking-[2px] uppercase">{{ $c[1] }}</p>
                        <p class="text-[14px] text-cloud/80">{{ $c[0] }}</p>
                    </div>
                </div>
                @endforeach
            </div>
